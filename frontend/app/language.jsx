
export const translations = {
    en: {
        // universal name 10
        term: "Term",
        editterm: "Edit",
        definition: "Definition",
        donebt: "Done",
        createbtn: "Create",
        enterterm: "Enter term",
        enterdefinition: "Enter definition",
        backbtn: "← Back",
        tryagainbtn: "Try again?",
        congradbtn: "Congratuation!, Want to try it again?",

        // login, register
        logout: "Logout",
        login: "Login",
        password: "Password",
        noaccount: "Don't have an account?",
        register: "Register",
        username: "Username",
        haveaccount: "Already have an account?",
        somethingwrong: "Something went wrong.",
        usernametaken: "Username already taken.",
        emailtaken: "Email already registered.",
        loading: "Loading...",

        // homepage more feature
        otherpeopleset: "Other People's Sets",
        createby: "Created by:",
        nopublicsets: "No public sets yet.",

        // yourlibrary more feature
        doneManaging: "Done",
        manageLibrary: "Manage Library",
        setpublic: "Public",
        setprivate: "Private",

        // AI feature
        askai: "Ask AI",
        aicreate: "Ask AI to create a set",
        topic: "Topic",
        topiceg: "e.g., brands, famous singers, planets",
        numterm: "Number of Terms",
        generatebtn: "Generate",
        genwait: "Generating...",


        // search bar 1
        searchbar: "Study set, puzzle, news",

        // side pannel 6
        home: "Home",
        library: "Library",
        createfolder: "Create Folder",
        yourFolders: "Your Folders",
        explore: "Explore",
        playcrossword: "CrossWord",

        // home page 5
        termsg: "term",
        termmul: "terms",
        recentfile: "Recent Created",
        flashcardTip: "Study Tip",
        noFolders: "No folders created yet.",

        // create set page 9
        need1term: "You need at least one term to create a study set.",
        createnewset: "Create a new learning set",
        entertitle: "Enter Title",
        enterdescription: "Add a description...",
        addimage: "Add Image",
        addcard: "+ Add more card",
        alway1card: "Alway add 1 card",
        card: "Card",
        cards: "Cards",

        //library page 1
        yourlibrary: "Your Library",

        // flashcard review page 8
        matchingcard: "Matching Card",
        fillIn: "Find The Term",
        puzzle: "Crossword Puzzle",
        loopTest: "Loop Test",
        terminset: "Term in this set",
        addorremove: "Add or remove term",
        editset: "Edit set",
        whatterm: "Term ?",

        // puzzle page 14
        playpuzzle: "Play crossword puzzle",
        nopuzzle: "No puzzle created yet.",
        across: "Across",
        down: "Down",
        checkanswer: "Check Answer!",
        createyourpuzzle: "Create your puzzle",
        createpuzzle: "Create puzzle",
        question: "Question:",
        answer: "Answer:",
        enterquestion: "Enter question...",
        enteranswer: "Enter answer...",
        addbt: "Add",
        enteryet: "Add a question and answer above to start",
        untitle: "Untitle",
        studyTips: [
            "Did you know your brain may absorb information better right before eating? Try reviewing your study set before a meal.",
            "Reviewing your notes before sleep can help lock the information into long-term memory.",
            "Teaching someone else what you learned is a powerful way to reinforce your knowledge.",
            "Handwriting notes can help you remember better than typing.",
            "Take short breaks every 25–30 minutes to keep your focus sharp."
        ]
    },
    vi: {
        // universal name 10
        term: "Thuật ngữ",
        editterm: "Sửa",
        definition: "Định nghĩa",
        donebt: "Xong",
        createbtn: "Thiết Lập",
        enterterm: "Nhập thuật ngữ",
        enterdefinition: "Nhập định nghĩa",
        backbtn: "← Quay lại",
        tryagainbtn: "Thử lại?",
        congradbtn: "Chúc mừng! Bạn muốn thử lại không?",

        // login, register
        logout: "Đăng xuất",
        login: "Đăng nhập",
        password: "Mật khẩu",
        noaccount: "Chưa có tài khoản?",
        register: "Đăng ký",
        username: "Tên người dùng",
        haveaccount: "Đã có tài khoản?",
        somethingwrong: "Đã xảy ra lỗi.",
        usernametaken: "Tên người dùng đã được sử dụng.",
        emailtaken: "Email đã được đăng ký.",
        loading: "Đang tải...",

        // homepage more feature
        otherpeopleset: "Bộ thẻ của người khác",
        createby: "Được tạo bởi:",
        nopublicsets: "Chưa có bộ thẻ công khai nào.",

        // yourlibrary more feature
        doneManaging: "Hoàn tất",
        manageLibrary: "Quản lý thư viện",
        setpublic: "Công khai",
        setprivate: "Riêng tư",

        // AI feature
        askai: "Hỏi AI",
        aicreate: "Yêu cầu AI tạo bộ thẻ",
        topic: "Chủ đề",
        topiceg: "ví dụ: thương hiệu, ca sĩ nổi tiếng, hành tinh",
        numterm: "Số lượng thuật ngữ",
        generatebtn: "Tạo",
        genwait: "Đang tạo...",


        // search bar 1
        searchbar: "Tệp học, ô chữ, tin tức",

        // side panel 6
        home: "Nhà",
        library: "Thư viện",
        createfolder: "Tạo Tệp Mới",
        yourFolders: "Tệp của bạn",
        explore: "Khám phá",
        playcrossword: "Trò Chơi Ô chữ",

        // home page 5
        termsg: "từ",
        termmul: "từ",
        recentfile: "Tạo gần đây",
        flashcardTip: "Mẹo học tập",
        noFolders: "Chưa có thư mục nào.",

        // create set page 9
        need1term: "Bạn cần it nhất 1 từ để tạo danh sách mới.",
        createnewset: "Tạo danh sách học mới",
        entertitle: "Nhập tiêu đề",
        enterdescription: "Thêm mô tả...",
        addimage: "Thêm ảnh",
        addcard: "+ Thêm thẻ",
        alway1card: "Luôn thêm 1 thẻ",
        card: "Thẻ",
        cards: "Thẻ",

        // library page 1
        yourlibrary: "Thư viện của bạn",

        // flashcard review page 7
        matchingcard: "Nối Từ",
        fillIn: "Tìm từ khoá",
        puzzle: "Ô chữ",
        loopTest: "Bài thi vòng lặp",
        terminset: "Danh sách các mục",
        addorremove: "Thêm hoặc xoá mục",
        editset: "Chỉnh sửa tệp",
        whatterm: "Là..?",

        // puzzle page 14
        playpuzzle: "Chơi ô chữ",
        nopuzzle: "Chưa có câu đố nào được tạo",
        across: "Ngang",
        down: "Dọc",
        checkanswer: "Kiểm tra!",
        createyourpuzzle: "Tạo câu đố",
        createpuzzle: "Tạo câu đố ô chữ",
        question: "Câu hỏi:",
        answer: "Đáp án:",
        enterquestion: "Nhập câu hỏi...",
        enteranswer: "Nhập đáp án..",
        addbt: "Thêm",
        enteryet: "Nhập câu hỏi và đáp án ở trên để bắt đầu",
        untitle: "Chưa đặt tên",
        studyTips: [
            "Bạn có biết não của bạn có thể tiếp thu thông tin tốt hơn ngay trước khi ăn? Hãy thử xem lại bộ từ vựng trước bữa ăn.",
            "Ôn lại ghi chú trước khi ngủ có thể giúp ghi nhớ lâu dài.",
            "Dạy lại cho người khác những gì bạn học được là cách hiệu quả để ghi nhớ.",
            "Viết tay giúp bạn ghi nhớ tốt hơn so với gõ máy.",
            "Nên nghỉ ngắn mỗi 25–30 phút để giữ sự tập trung."
        ]
    }

};
